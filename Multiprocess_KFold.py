...
def kfold_async(x,y,k,task,cv):
    ...
    for i in range(begin,end):
        ...
        gc.fit(x_train,y_train,x_val,y_val)
...
def get_config():
    ...
    for i in range(4):
        config["estimator_configs"].append({"n_fold":5,"type":"GradientBoostingRegressor","n_estimators":200})
    # for i in range(4):
        # config["estimator_configs"].append({"n_fold":5,"type":"GradientBoostingRegressor","n_estimators":100})
    return config
