...
class gcForest(object):
    ...
    def fit(self,x,y,x_val,y_val):
        ...
        while deepth<self.max_layers:
            ...
            # self.LOGGER.info(y_test_pred[0:50])
            self.LOGGER.info("R2={:.4f}, MSE={:.4f}, RMSE={:.4f}, MAE={:.4f}, pearsonr={:.4f}".format(r2_score(y_val,y_test_pred),\
            mean_squared_error(y_val,y_test_pred),np.sqrt(mean_squared_error(y_val,y_test_pred)),mean_absolute_error(y_val,y_test_pred),pearsonr(y_val,y_test_pred)[0]))

            if deepth>=1:
                self.LOGGER.info("layers num:{}".format(len(self.layers)))
                break
            ...
            x2,y2=resample(x_error_1,y_error_1,int(s_r*len(index1)))
            x3,y3=resample(x_error_2,y_error_2,int(s_r*len(index2)))
            correct_num=int(a*n_train*0.7)
